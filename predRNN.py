...
from keras.layers.STLSTM import *
...
def input_fn():
  ...
  labels.prefetch(1)
  train_dataset = train_dataset.apply(tf.contrib.data.shuffle_and_repeat(BUFFER_SIZE, EPOCHS))
  labels.prefetch(1)
  print("Dataset creation took {}".format(time.time() - processing_start))
...
with keras.utils.custom_object_scope({'StackedSTLSTMCells':StackedSTLSTMCells,
  ...
  predRNN_multi_gpu = make_parallel(predRNN, NUM_GPU)
  optimizer = keras.optimizers.Adam(lr=0.001)
  predRNN_multi_gpu.compile(optimizer = optimizer,
                            loss = l1_l2_loss,
                            metrics = [tf.keras.metrics.mse])

  predRNN_multi_gpu.summary()
  predRNN_multi_gpu.fit(train_dataset, epochs=EPOCHS, callbacks=callbacks,
                        validation_data=valid_dataset, steps_per_epoch=BATCHES_PER_EPOCH,
                        validation_steps=BATCHES_PER_EPOCH_VALID)

  #keras_estimator = keras.estimator.model_to_estimator(keras_model=predRNN, config=config, model_dir='./model_dir2')
  #keras_estimator.train(input_fn=input_fn, steps=BATCHES_PER_EPOCH * EPOCHS)
       
  #predRNN.fit(train_dataset, epochs=EPOCHS, steps_per_epoch=BATCHES_PER_EPOCH)
